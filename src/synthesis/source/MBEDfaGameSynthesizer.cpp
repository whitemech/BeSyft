/*
*
* This file implements the MBEDfaGameSynthesizer type
* used for Joker synthesis
*
*/
...
namespace Syft {
    ...
    MBEDfaGameSynthesizer::MBEDfaGameSynthesizer(SymbolicStateDfa spec, SymbolicStateDfa neg_env,
        ...
        // transition vector of the restricted domain (ENV)
        transition_vector_star_ = var_mgr_neg_->make_compose_vector(neg_env.automaton_id(),
                                                            neg_env.transition_function());
        ...
        CUDD::BDD input_cube_neg = var_mgr_neg_->input_cube();   // i.e. X_restricted
        ...
        if (starting_player_ == Player::Environment) {
            ...
            if (protagonist_player_ == Player::Environment) {
                ...
                quantify_independent_variables_ = std::make_unique<Forall>(input_cube); //FORALL X
                //quantify_independent_variables_star_ = std::make_unique<Exists>(input_cube); // EXISTS X
                quantify_independent_variables_star_ = std::make_unique<Exists>(input_cube_neg); // EXISTS X
                quantify_non_state_variables_ = std::make_unique<Exists>(output_cube); // EXISTS/FOR ALL X s.t. EXISTS Y
            }
        }

    }

    CUDD::BDD MBEDfaGameSynthesizer::preimage(
    ...
    CUDD::BDD MBEDfaGameSynthesizer::preimage_star(
        const CUDD::BDD &winning_states) const {
            //transitions taken from the transition vector of the restriced BDD
            CUDD::BDD winning_transitions = 
                winning_states.VectorCompose(transition_vector_star_); //(ENV)    transition_vector_
                
            CUDD::BDD aiuto = quantify_independent_variables_star_ -> apply(winning_transitions); 
            
            return aiuto;
    }

    CUDD::BDD MBEDfaGameSynthesizer::project_into_states(
        const CUDD::BDD& winning_moves) const {
            return quantify_non_state_variables_->apply(winning_moves);
        }
    
    bool MBEDfaGameSynthesizer::includes_initial_state(const CUDD::BDD &winning_states) const {
        std::vector<int> copy(initial_vector_);
        return winning_states.Eval(copy.data()).IsOne();
    }

    std::unordered_map<int, CUDD::BDD> MBEDfaGameSynthesizer::synthesize_strategy(
        const CUDD::BDD &winning_moves) const {
